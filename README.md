...
CLI 失败结果现在也会带 `cli_failure_kind` 和 `cli_recovery_hint`。当前默认 `triage` / `review` 已经走隔离版 Claude；如果你刻意切回非隔离 `claude_router` 并且又卡住，可以优先试 `OPENCLAW_ASSIGN_TRIAGE_LOCAL=claude_router_isolated`，或者直接切到 `mission_control_openclaw_triage` / `mission_control_openclaw_default`。
